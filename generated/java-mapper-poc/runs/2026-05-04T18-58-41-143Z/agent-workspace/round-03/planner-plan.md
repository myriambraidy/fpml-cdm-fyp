...
Runtime fixtures (XML files): fx-ex01-fx-spot.xml, fx-ex02-spot-cross-w-side-rates.xml, fx-ex03-fx-fwd.xml, fx-ex04-fx-fwd-w-settlement.xml, fx-ex05-fx-fwd-w-ssi.xml, fx-ex06-fx-fwd-w-splits.xml, fx-ex07-non-deliverable-forward.xml
...
- `cdm.product.template.Payout` (Payout container) - No valid `SettlementPayout` in CDM Java 6.7.0
- `cdm.observable.asset.PriceQuantity` (Price/quantity details)
...
5. **MapFxCoreDetailsModelToSettlementPayout** - Maps core FX details to settlement payouts (Alternative: Uses PriceQuantity for FX spot/fwd details)
6. **MapFxSingleLegPriceQuantityList** - Maps price/quantity data for trades
7. **MapPayerReceiverModelToCounterpartyList** - Shared helper for counterparty mapping
8. **MapPayerReceiver** - Shared helper for payer/receiver mapping
9. **MapPayerReceiverToAccountPartyReference** - Shared helper for party reference mapping
...
5. `cdm.observable.asset.ResolvablePriceQuantity` is not part of CDM Java; `cdm.observable.asset.PriceQuantity` is used instead
6. `cdm.product.common.settlement.SettlementPayout` is not part of CDM Java; **`cdm.product.template.SettlementPayout` also does not exist** - Payout with a SettlementPayout member is not possible. The implementation will build `Payout` with a different type or an empty variant when settlement details are required.
...
No change is proposed. The current implementation group `fx-single-leg` is appropriate for this run, as it addresses the runtime supported fixtures and avoids unsupported or future-phase products.