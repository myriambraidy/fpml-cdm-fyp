...
- `NdfValidatorTest` - Tests NDF-specific rules

### Integration tests
- `FpmlFxMapperIntegrationTest` - Tests complete pipeline from FpML XML to CDM JSON
- `FxSingleLegEndToEndTest` - Tests full round-trip conversion for each supported fixture (fx-ex01 through fx-ex07)
...
- Ensure data normalization (e.g., trailing Z in dates) is applied correctly to `tradeDate`, `valueDate`, `settlementDate`, `fixingDate` as appropriate
...
- Trade date normalization must consistently remove trailing 'Z' from dates (per TR-002), including `valueDate`, `settlementDate`, `fixingDate` where present
