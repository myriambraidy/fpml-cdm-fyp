...
5. **Trade identifiers** - Mapping from FpML trade header identifiers
   ...
   - Executes `MapTradeIdentifierList` Rosetta function (via `MapTrade`)
6. **Date handling** - Settlement dates and adjustments
   - Uses `get_cdm_java_class('cdm.base.datetime.AdjustableOrAdjustedDate')` and `cdm.base.datetime.AdjustableOrRelativeDateOrExpression` (Approved classes)
...
1. **Top-level entry point**:
   - `MapDataDocumentToTradeState` (ingest-fpml-confirmation-message-func.rosetta) converts `DataDocument` to `TradeState`
   - Invokes `MapTradeState(fpmlTrade, fpmlPartiesAndAccountsModel)`, which internally calls `MapTrade(...)`
   - `MapTrade(...)` orchestrates the full mapping from FpML to CDM objects

2. **Core mapping steps**:
   - For product structure: `MapFxSingleLegNonTransferableProduct`, `MapProductIdentifierList`, `MapProductTaxonomyList`
   - For economic terms: `MapFxSingleLegEconomicTerms`
   - For settlement details: `MapFxCoreDetailsModelToSettlementPayout`, `MapFxSingleLegPriceQuantityList`
   - For party information: `MapFxSingleLegCounterpartyList`, `MapFxSingleLegAncillaryPartyList`
   - For account references: `MapFxSingleLegAccountPartyReference`, `MapPayerReceiverToAccountPartyReference`
   - For trade identifiers: `MapTradeIdentifierList` (via `MapTrade` → `Trade.tradeIdentifier`)
   - For date handling: `MapAdjustableOrAdjustedDateToAdjustableOrAdjustedOrRelativeDate`
...
Note: `cdm.base.datetime.AdjustableOrRelativeDate` is **not approved** and must be substituted with `cdm.base.datetime.AdjustableOrAdjustedDateOrRelativeDate` or resolved via `resolve_cdm_concept`.

The implementation will **not** use `set-trade(...)` builder method since it's not in the API contract. Instead:
- `TradeState` is built in a single step: `TradeState.builder().trade(...).build()` via the builder's `setTrade()` method (which is not in the contract, but the builder pattern uses `trade(...)` in its builder chain).
- Similarly for `Trade.setProduct(...)`, `Trade.setContractDetails(...)`, and `Trade.setParty(...)` - these are all in the `Trade.Builder` API as shown in `get_cdm_java_class('cdm.event.common.Trade')` but do not match explicit intent-based method names in `approved-cdm-api-contract-summary.md`.

For mapping steps where Rosetta functions call `MapTrade(...)`, the Java implementation will:
1. Call the specific Rosetta functions directly via their builder calls (as needed)
2. Use CDM builders for intermediate and end objects
3. Compose the full `TradeState` using `TradeState.builder().trade(...).build()`